...
fn solve(map: BTreeMap<u64, u64>, range: usize) -> u64 {
    ...
    for _ in 0..range {
        for (item, count) in start_map.clone().iter() {
            ...
            if *item == 0 {
                condition_one(&mut start_map, *item, *count);
                continue;
            }

            if item.to_string().len() % 2 == 0 {
                condition_two(&mut start_map, *item, *count);
                continue;
            }

            condition_three(&mut start_map, *item, *count);
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_part1() {
        assert_eq!(solve(parse_input(TEST_INPUT_PATH), 25), 55312);
    }
}
